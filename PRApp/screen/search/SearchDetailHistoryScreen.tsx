...
const SearchDetailHistoryScreen = ({navigation, route}: INavigationRoute) => {
  const uri = `${webviewResource.uri}/search/detail/history?id=${route?.params?.id}&app=SearchDetailHistoryScreen`;
  ...
  const handleOnMessage = (msg: any) => {
    ...
    if (JSON.parse(msg?.nativeEvent?.data)?.goBack === 'SearchDetailHistoryScreen') {
      navigation.goBack();
    }
  };
...
export default SearchDetailHistoryScreen;
